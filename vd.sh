...
# --- UL-PRO SYSTEM HARDENED PREMIUM THEME ---
...
MAX_ATTEMPTS=3
SCRIPT_VERSION="5.5-UL-PRO-HARDENED"

# Auto Dependency Core Guard
...
# Strict Verification Dynamic Loop
...
# Deep Validation (Bypass Injection Mitigation)
if [ -z "$GLOBAL_DAYS_LEFT" ] || [ "$GLOBAL_DAYS_LEFT" -eq 0 ]; then
    echo -e "${RED}🔒 Security Fault: Unauthorized Runtime State Detected.${RESET}"
    exit 1
fi
