...
if [ ! -d "$DOWNLOAD_DIR" ]; then
    echo -e "${YELLOW}🔑 Requesting Storage Permission... Please Allow it!${RESET}"
...
while true; do
    ...
    printf "${WHITE}👉 Choose an option (1-8): ${RESET}"
    read op
    ...
    if [ "$op" = "7" ]; then
        ...
        if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
            # File complete clear ba remove kora
            > "$HISTORY_FILE"
    ...
    if [[ ! "$op" =~ ^[1-5]$ ]]; then
        echo -e "\n${RED}❌ Invalid Option! Please select between 1 and 8.${RESET}"
        echo -e "${WHITE}Press ENTER to try again...${RESET}"
        read
        continue
    fi
    ...
    # --- Invalid URL & Empty URL Detection ---
    if [ -z "$url" ]; then
    ...
    case $op in
        ...
        3) $YTDL $PLAYLIST_ARG -x --audio-format mp3 -o "$OUT" "$url" ; type_str="MP3 Audio" ;;
        4)
    ...
    # --- Error Handling & Clean History Logging ---
    if [ $? -eq 0 ]; then
        ...
        echo -e "\n${RED}❌ Action Failed! Reason could be: Invalid link, Network drop or Restricted Video.${RESET}"
    fi
