...
# =================================================================
# 🛡️ UL-PRO MAXIMUM FORTIFIED SYSTEM | DYNAMIC CLOUD DATABASE v10.0
# =================================================================

# --- Premium Core Color Matrices & Dynamic Assets ---
...
# --- 🟢 LIVE FIREBASE REALTIME DATABASE URL CONNECTED 🟢 ---
DB_URL="https://sweety-d0130-default-rtdb.firebaseio.com"
...
MAX_ATTEMPTS=3
SCRIPT_VERSION="10.0-DB-HARDENED"

# --- Automated Dependency System Core Guard ---
...
check_device_lock() {
    ...
    if [ -f "$SESSION_TOKEN" ]; then
        ...
        SERVER_DATA=$(curl -s --fail --max-time 10 "${DB_URL}/users/${SAVED_KEY}.json")
        if [ $? -eq 0 ] && [ "$SERVER_DATA" != "null" ] && [ ! -z "$SERVER_DATA" ]; then
            USER_STATUS=$(echo "$SERVER_DATA" | jq -r ".status")
            USER_EXPIRY=$(echo "$SERVER_DATA" | jq -r ".expiry")
            ALLOWED_HW_ID=$(echo "$SERVER_DATA" | jq -r ".device_id")
...
# --- Cloud Database Gateway Loop ---
if ! check_device_lock; then
    attempts=0
    ADMIN_NOTICE=$(curl -s --fail --max-time 10 "${DB_URL}/notice.json" | tr -d '"')
    ...
    while true; do
        clear
        echo -e "${CYAN}${BOLD}╔═══════════════════════════════════════════════╗${RESET}"
        echo -e "${CYAN}${BOLD}║  🚀 UL-PRO ELITE | CLOUD REVALIDATION ENGINE  ║${RESET}"
        echo -e "${CYAN}${BOLD}╚═══════════════════════════════════════════════╝${RESET}"
        ...
        if [ $attempts -gt 0 ]; then
            ...
            echo -e "${YELLOW} 👉 Press ENTER to open Facebook and follow owner for getting active keys!${RESET}"
            echo -e "${DIM} ───────────────────────────────────────────────${RESET}"
            read -p ""
        ...
        USER_DATA=$(curl -s --fail --max-time 10 "${DB_URL}/users/${user_code}.json")

        if [ "$USER_DATA" = "null" ] || [ -z "$user_code" ] || [ -z "$USER_DATA" ]; then
            attempts=$((attempts + 1))
            if [ $attempts -ge $MAX_ATTEMPTS ]; then
                echo $(($(date +%s) + 3600)) > "$BAN_FILE"; exit 1
            fi
        else
            USER_STATUS=$(echo "$USER_DATA" | jq -r ".status")
            USER_EXPIRY=$(echo "$USER_DATA" | jq -r ".expiry")
            ALLOWED_HW_ID=$(echo "$USER_DATA" | jq -r ".device_id")
            USER_REASON=$(echo "$USER_DATA" | jq -r ".reason")

            if [ "$USER_STATUS" = "blocked" ]; then
                echo -e "${RED}⛔ ACCESS TERMINATED! Reason: $USER_REASON${RESET}"
                echo -e "${YELLOW}👉 Press ENTER to contact owner on Facebook...${RESET}"; read
                termux-open-url "$FB_URL" 2>/dev/null || am start -a android.intent.action.VIEW -d "$FB_URL" &>/dev/null
                exit 1
            elif [[ "$(date +%Y-%m-%d)" > "$USER_EXPIRY" ]]; then
                echo -e "${RED}⛔ SYSTEM KEY EXPIRED! Update credentials.${RESET}"
                echo -e "${YELLOW}👉 Press ENTER to get new updated key from FB Bio...${RESET}"; read
                termux-open-url "$FB_URL" 2>/dev/null || am start -a android.intent.action.VIEW -d "$FB_URL" &>/dev/null
                exit 1
            elif [ "$USER_STATUS" = "active" ]; then
                if [ "$ALLOWED_HW_ID" = "null" ]; then
                    echo -e "${YELLOW}⚠️ Mapping hardware signature token to cloud node...${RESET}"
                    curl -s -X PATCH -d "{\"device_id\": \"$CURRENT_HW_ID\"}" "${DB_URL}/users/${user_code}.json" > /dev/null
                elif [ "$ALLOWED_HW_ID" != "$CURRENT_HW_ID" ]; then
                    echo -e "${RED}⛔ HARDWARE MISMATCH! Account mapped to other hardware.${RESET}"
                    echo -e "${YELLOW}👉 Press ENTER to inform owner on FB...${RESET}"; read
                    termux-open-url "$FB_URL" 2>/dev/null || am start -a android.intent.action.VIEW -d "$FB_URL" &>/dev/null
                    exit 1
                fi
...
while true; do
    ...
    [ "$op" = "0" ] && { echo -e "\n${RED}🔒 Session closed.${RESET}"; rm -f "$SESSION_TOKEN"; exit 0; }
